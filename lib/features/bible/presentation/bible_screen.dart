...
import '../../../core/services/gradio_client.dart';
...
class _BibleScreenState extends ConsumerState<BibleScreen> {
  ...
  List<BibleVerseEntity> _verses = [];
  final _referenceController = TextEditingController();
  final _searchController = TextEditingController();
  List<BibleVerseEntity> _searchResults = [];
  ...
  Map<int, BibleHighlightEntity> _highlights = {};
  ...
  Future<void> _openChapter(BibleBookEntity book, int chapterNumber) async {
    setState(() {
      _loadingVerses = true;
      _error = null;
    });
  ...
  /// TtsGenerationException carries a [GradioErrorType] — translate the
  /// common ones into something a listener actually understands instead
  /// of a raw exception string, since "the free voice service is asleep,
  /// try again shortly" is actionable and "GradioClientException(...)"
  /// isn't.
  String _friendlyTtsError(TtsGenerationException e) {
    switch (e.type) {
      case GradioErrorType.spaceStarting:
        return 'The voice service is waking up — this can take a minute on the free tier. Try again shortly.';
      case GradioErrorType.rateLimited:
        return 'The voice service is busy right now. Please try again in a moment.';
      case GradioErrorType.timeout:
        return 'The voice service took too long to respond. Please try again.';
      case GradioErrorType.network:
        return 'Network error while generating audio — check your connection and try again.';
      case GradioErrorType.serverError:
      case GradioErrorType.unknown:
  ...
  void _copyVerse(BibleVerseEntity v) {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return FutureBuilder<List<BibleBookEntity>>(
      ...
      builder: (context, snapshot) {
        ...
        return ListView(
          children: [
            FutureBuilder<BibleReadingStreakEntity>(
            ...
            if (onContinueReading != null)
              FutureBuilder<BibleReadingProgressEntity?>(

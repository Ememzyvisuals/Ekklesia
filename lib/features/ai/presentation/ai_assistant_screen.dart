...
class _AiAssistantScreenState extends State<AiAssistantScreen> {
  ...
  Future<void> _loadHistory() async {
    ...
    try {
      final messages = await ConversationRepository.instance
          .sessionMessages(uid, _sessionId)
          .first
      ...
      // Offline on first open with nothing cached yet — not fatal, just
      // start with an empty history for today; ConversationWorker's queue
      // still covers anything sent from here on.
  ...
  /// than WazobiaVoice's per-call limit — see AppConfig.wazobiaVoiceMaxChars)
  /// are saved as separate numbered parts rather than merged into one
  /// file; merging audio client-side is out of scope for this pass.
  ...
  Future<void> _listenTo(String text, int index) async {
    ...
    try {
      // AI replies can easily run past WazobiaVoice's fixed generation
      // length, so this splits into safe-sized chunks and plays them
      // back-to-back rather than sending the whole reply in one call.
      final chunks = TtsService.instance.synthesizeChunks(
        text: text,
        language: EkklesiaLanguage.english,
      );
        ...
        actions: [
          FutureBuilder<bool>(
            ...
            builder: (context, hasKeySnapshot) {
              if (hasKeySnapshot.data == true) return const SizedBox.shrink();
              return FutureBuilder<int>(
                future: GroqUsageService.instance.getRemainingToday(),

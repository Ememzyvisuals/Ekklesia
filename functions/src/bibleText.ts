...
// Same 66-book slug map as lib/core/services/bible_service.dart's
// _bookSlugs — kept in sync manually (see config.ts's header note on why
// this repo doesn't have cross-language codegen yet).
...
/**
 * English-only fetch (mirrors VerseWorker._generateAndStore, which also
 * only fetches English text — other languages are read client-side by
 * BibleService when the user opens the Bible tab). Returns null on any
 * failure so the caller can still write a reference-only doc, matching
 * the client's existing fallback behavior rather than failing the whole
 * scheduled run over a transient CDN error.
 */
